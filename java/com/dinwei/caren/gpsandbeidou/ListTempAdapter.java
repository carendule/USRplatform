...
public class ListTempAdapter extends BaseAdapter {
    ...
    public ListTempAdapter(Context context){
    ...
    @Override
    public int getCount() {
        return HistoryTemp.historyTemps.size();
    }
    ...
    public View getView(int position, View view, ViewGroup viewGroup) {
        ListTempAdapter.ListItemView listItemView = null;
        if (view == null){
        ...
        if (HistoryTemp.historyTemps.isEmpty()){
            listItemView.listtime.setText("无数据");
            listItemView.listvalue.setText("无数据");
        }else{
            listItemView.listtime.setText(unix2data(HistoryTemp.historyTemps.get(position).getChangetime()));
            listItemView.listvalue.setText(String.valueOf(HistoryTemp.historyTemps.get(position).getTemp()));
